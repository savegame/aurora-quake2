 ...
 *
 * В проекте собирается под AURORA_MALIIT (CMake: pkg_check_modules
 * glib-2.0/gio-2.0/gio-unix-2.0); без дефайна — весь файл компилируется
 * в no-op стабы (init возвращает false), host-сборка без glib не ломается
 * (по образцу aurora_keepalive.c / dbus-1).
 */
...
void maliit_client_set_orientation( int32_t angle_degrees )
{
	if( !g.ready ) return;
	server_call( "appOrientationAboutToChange",
		g_variant_new( "(i)", angle_degrees ) );
	server_call( "appOrientationChanged",
		g_variant_new( "(i)", angle_degrees ) );
}

#else /* !AURORA_MALIIT — стабы: API есть, но не работает */
...
{
	(void)cb;
	fprintf( stderr, "maliit_client: собран без glib (AURORA_MALIIT нет) — OSK недоступна\n" );
	return false;
}

void maliit_client_shutdown( void ) {}
void maliit_client_pump( void ) {}
bool maliit_client_is_ready( void )   { return false; }
bool maliit_client_is_visible( void ) { return false; }
void maliit_client_show( void ) {}
void maliit_client_hide( void ) {}
void maliit_client_focus_in( void ) {}
void maliit_client_focus_out( void ) {}
void maliit_client_set_content_type( maliit_content_type_t type ) { (void)type; }
void maliit_client_set_prediction( bool enabled )   { (void)enabled; }
void maliit_client_set_correction( bool enabled )   { (void)enabled; }
void maliit_client_set_autocaps( bool enabled )     { (void)enabled; }
void maliit_client_set_hidden_text( bool hidden )   { (void)hidden; }
void maliit_client_set_password_mode( bool password ) { (void)password; }
void maliit_client_set_cursor_rect( int32_t x, int32_t y, int32_t w, int32_t h )
{
...
void maliit_client_set_surrounding_text( const char *text, int32_t cursor_pos )
{
	(void)text; (void)cursor_pos;
}
void maliit_client_set_orientation( int32_t angle_degrees ) { (void)angle_degrees; }
